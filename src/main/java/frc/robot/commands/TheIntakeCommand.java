// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commands;

import edu.wpi.first.wpilibj.GenericHID.RumbleType;
import edu.wpi.first.wpilibj.Timer;
import edu.wpi.first.wpilibj.XboxController;
import edu.wpi.first.wpilibj2.command.Command;
import frc.robot.Constants;
import frc.robot.subsystems.IntakerSubsystem;
import frc.robot.subsystems.OverBumperSubsystem;

public class TheIntakeCommand extends Command {
  private IntakerSubsystem m_intakeSubsystem;
  private OverBumperSubsystem m_overBumperSubsystem;
  private XboxController Driver;

  public final Timer timer = new Timer();
  
  /** Creates a new TheIntakeCommand. */
  public TheIntakeCommand(IntakerSubsystem int_sub, OverBumperSubsystem theArmThing, XboxController driver) {
    m_intakeSubsystem = int_sub;
    m_overBumperSubsystem = theArmThing;
    this.Driver = driver;

    addRequirements(int_sub, theArmThing);
  }

  // Called when the command is initially scheduled.
  @Override
  public void initialize() {
    Driver.setRumble(RumbleType.kBothRumble, 0);
    m_overBumperSubsystem.setUporDown(false, Constants.Arm.ArmDown);
  }

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {
    if (!m_intakeSubsystem.intakeBreak()) {
      m_intakeSubsystem.SetAll();
    } else {
      m_intakeSubsystem.StopAll();
      Driver.setRumble(RumbleType.kBothRumble, Constants.controllerRumble);
    }
  }

  // Called once the command ends or is interrupted.
  @Override
  public void end(boolean interrupted) {
    m_intakeSubsystem.StopAll();
    Driver.setRumble(RumbleType.kBothRumble, 0);
    m_overBumperSubsystem.setUporDown(true, Constants.Arm.ArmUp);
  }

  // Returns true when the command should end.
  @Override
  public boolean isFinished() {
    return false;
  }
}
